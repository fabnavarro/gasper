...
```
`usroads-48` is composed of the matrix `sA`, coordinates `xy` (if present), `dim` the number of rows, columns and numerically nonzero elements and `info` about the matrix (stored in a temporary folder as a plain text file) that can be display via `file.show(usroads-48$info)` for example or in the console:

```{r}
cat(readLines(`usroads-48`$info), sep = "\n")
```

It is also possible to plot a (planar) graph and plot signals defined on top of the graph. For example :
```{r,fig.show='hold'}
