...
Information about the matrix that can be display via `file.show(paste(grid1$temp,"grid1",sep=""))` or in the console:
```{r, eval = FALSE}
...
x_table <- knitr::kables(
  list(df1,df2), 
  format = "latex", 
  caption = "Matrix Information and Matrix Properties")
...
Among the diverse types of graphs, such as undirected, directed, weighted, bipartite, and multigraphs, each offers distinct analytical advantages tailored to specific contexts. This vignette is devoted to undirected graphs where edges link two vertices symmetrically, often with weighted values to express connection strength or intensity.  For instance, in a road network graph, the weights might correspond to the length of each road segment. 
...
df <-kableExtra::kbl(res, 
                    valign = 't',
                    format = "latex", booktabs = T, caption="Uniform vs Dependent" )
