...
attributes(netz4504)
```
The output is stored (in a temporary folder) as a data.frame list composed of "sA", possibly coordinates "xy", "dim" the number of rows, columns and numerically nonzero elements and "info"" about the matrix. 
...
An alternative to avoid frame calculation is given by the `forward_tf` function which provides a fast SGWT. For exemple:
```{r}
# wcf <- forward_tf(f, evalues, evectors, b=b)
```

The optimal threshold is then determined by minimizing the SURE (using the Donoho and Johnstone's trick @donoho1995adapting which remains valid here, see @de2019data).
```{r}
