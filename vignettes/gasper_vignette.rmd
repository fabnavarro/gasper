...
```{r, fig.show='hold'}
f <- randsignal(eta=0.01, k=5, A=grid1$sA)
plot_graph(grid1)
plot_signal(grid1, f, size = 50*f)
```
...
```{r,fig.show='hold'}
plot_signal(grid1, f, size = 3)
plot_signal(grid1, tilde_f, size = 3)
```
...
The optimal threshold is then determined by minimizing the SURE (using Donoho and Johnstone's trick @donoho1995adapting which remains valid here, see @de2019data).
```{r}
...
We can plot MSE risk and its SURE estimates as a function of the threshold parameter (assuming that $\sigma$ is known)
```{r}
...
Finally, the synthesis allows us to determine the resulting estimators of $f$, \emph{i.e.}, the one that minimizes the unknown MSE risk and the one that minimizes the SURE.
```{r}
